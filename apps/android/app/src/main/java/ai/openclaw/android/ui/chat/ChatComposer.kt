...
import ai.openclaw.android.chat.InputHistoryStore
...
fun ChatComposer(
  ...
  // Input history for ↑/↓ navigation (like terminal) - persisted across app sessions
  ...
  LaunchedEffect(Unit) {
    val stored = InputHistoryStore.load(context)
    inputHistory.clear()
    inputHistory.addAll(stored)
  }
      ...
      Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        if (pendingRunCount > 0) {
          ...
          FilledTonalIconButton(onClick = {
            val text = input.trim()
            if (text.isNotEmpty()) {
              // Add to input history (avoid duplicates of last entry) and persist
              InputHistoryStore.addEntry(context, inputHistory, text)
